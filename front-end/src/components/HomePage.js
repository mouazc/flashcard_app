...
const HomePage = () => {
  ...
  const fetchCategoriesAndFlashcards = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8080/flashcard_app/get_categories_and_flashcards.php",
        {
  ...
  const handlePractice = () => {
    navigate("/practice", { state: { selectedCategory } });
  };
  ...
  return (
    <div className="homepage-container">
      ...
      <div className="footer">© {new Date().getFullYear()} UMD. All rights reserved.</div>
    </div>
  );
};
